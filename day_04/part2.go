...
func main() {
	...
	winDraw, loserBoard := findLoser(drawOrder, bingoBoards)
	if loserBoard == nil {
		log.Fatalf("Failed to find a winning board.")
	}
	unmarked := loserBoard.unmarkedSquares()
...
func findLoser(draws []string, boards []*bingoBoard) (string, *bingoBoard) {
	losers := []*bingoBoard{}
	for _, b := range boards {
		losers = append(losers, b)
	}
	for _, d := range draws {
		newLosers := []*bingoBoard{}
		for _, b := range losers {
			...
			newLosers = append(newLosers, b)
		}
		if len(newLosers) == 0 && len(losers) == 1 {
			return d, losers[0]
		}
		losers = newLosers
		fmt.Printf("Marking %v, Num losers: %d\n", d, len(losers))
