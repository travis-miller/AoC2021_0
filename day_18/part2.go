...
func main() {
	...
	for i := 0; i < len(sNums); i++ {
		for j := i + 1; j < len(sNums); j++ {
			mag := sNums[i].sum(sNums[j]).magnitude()
			if mag > maxMag {
				maxMag = mag
			}
			mag = sNums[j].sum(sNums[i]).magnitude()
			if mag > maxMag {
				maxMag = mag
			}
		}
...
func (n *snailNumber) sum(other *snailNumber) *snailNumber {
	...
	nCopy := parseSnailNumber(n.String())
	otherCopy := parseSnailNumber(other.String())
	newNum := &snailNumber{childLeft: nCopy, childRight: otherCopy}
	nCopy.parent = newNum
	otherCopy.parent = newNum
	newNum.reduce()
	return newNum
}
